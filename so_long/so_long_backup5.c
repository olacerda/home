...
/*   Updated: 2025/09/22 08:12:40 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sheet_to_image_convertor(t_sheet *src, t_image *dst, t_image *background, int sprite_column, int sprite_line, t_playerinfo *play)
{
	int	ints_per_line;
	int	img_line;
	int src_idx;
	...
	int	background_line = difference_column * (background->bpp / 32);
	int	background_column = difference_line * background->sizeline;
	...
	int	background_line_limit = (background->tall * background->sizeline);
	...
	while (img_line < dst->tall)
	{
		...
		background_index += background->sizeline;
		background_column += background->sizeline;
		background_column_limit += background->sizeline;
		if (background_index >= background_line_limit)
			background_index -+ background_line_limit
		img_line++;
