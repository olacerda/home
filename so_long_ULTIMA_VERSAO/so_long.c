...
/*   Updated: 2025/09/26 07:20:02 by otlacerd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_game(t_all *all)
{
	...
	{
		if ((check_player_range2(all, 'C') == 1) && (all->states->full_colectables == 0))
		{
			write(1, "\a", 1);
			all->play->colected++;
		...
		else if ((check_player_range2(all, 'R') == 1) && all->play->R == 0)
		{
			write(1, "\a", 1);
			// static int flag = 1;
			// if (flag == 1)
...
int key_hook(int keycode, void *arg)
{
    ...
    if ((keycode >= 0 && keycode <= 25) || (keycode >= 97 && keycode <= 122))
    {
        char c[2] = { (char)keycode, '\0' };
        int len = strlen(all->game->writed);
        if (len < 9999)
        {
            all->game->writed[len] = c[0];
            all->game->writed[len+1] = '\0';
            mlx_string_put(all->mlx, all->window_terminal, x, y, 16711680, c);
            x += 10; // avança na linha
            if (x > 300) { x = 10; y += 20; } // nova linha se chegar na borda
        }
    }
    if (keycode == 36) // Enter
...
void open_terminal(t_all *all)
{
    if (!all->window_terminal)
    {
        ...
        // Opcional: desenhar algo inicial
        mlx_string_put(all->mlx, all->window_terminal, 10, 10, 16711680, "Terminal pronto...");
    }
}
...
// 			flag = 0;
// 		}
// 		// mlx_string_put(all->mlx, all->window_terminal, 100, 100, 16711680, "Isso e um teste");
// 		all->states->terminal = 0;
// 	}
...
void	general_settings(t_all *all)
{
	...
	all->game->real_elements = "PEC10RXITO";
	if (all->images)
