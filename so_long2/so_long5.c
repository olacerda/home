...
void	move_player(t_all *all, int *key_state, int yes, int no)
{
	...
	speed = all->game->speed + (2 * (-(all->game->speed) * (yes == 0)));
	sped2 = all->game->speed2 + (2 * (-(all->game->speed2) * (yes == 0)));
...
int	callback(t_all *all)
{
	...
	if (all->states->terminal != 1)
	{
		...
		move_player(all, keycode, yes, no);
		keycode[0] = all->states->key_s;
		keycode[1] = all->states->key_d;
		move_player(all, keycode, no, yes);
	}
...
int	cmp_msg(t_all * all, char *string1, char *string2, int limit)
{
	...
	if (index1 > 0)
		index1--;
	printf("Retornando: %c   menos    %c   resultado:  %d\n\n", string1[index1], string2[index2], string1[index1] - string2[index2]);
